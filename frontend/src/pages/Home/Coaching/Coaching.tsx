...
export default function Coaching() {
  ...
  const { data: dataMembership, loading: loadingMembership } =
    useGetMyMembershipQuery({
      fetchPolicy: "no-cache",
    });
  ...
  const memberShipExpired =
    new Date(myMembership?.endDate).getTime() < new Date().getTime();
  ...
  if (loadingCoach || loadingMembership)
    return (
  ...
  return (
    <section className="w-full h-full bg-white rounded-2xl pt-5">
      <section className="w-full h-full flex flex-col justify-start items-start mx-10 pb-8 gap-4">
        <ConfirmModal
          isOpen={openConfirm}
