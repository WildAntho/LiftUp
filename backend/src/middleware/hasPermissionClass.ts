import { createResolverClassMiddlewareDecorator, PubSub } from "type-graphql";
...
export function HasPermissionClass(
  permissions: string[],
  mode: "AND" | "OR" = "AND"
) {
  return createResolverClassMiddlewareDecorator<FullContext>(
    async ({ context }, next) => {
